            ...
            for mb_idx, start in enumerate(minibatch_iter):
                ...
                with accelerator.accumulate(agent):
                    ...
                    if is_critic_warmup:
                        ...
                        pg_loss = torch.zeros_like(v_loss)  # Not used
                        entropy_loss = torch.zeros_like(v_loss)  # Not used
                    ...
                    if not is_critic_warmup:
                        del newlogprob, newvalue, entropy_tensor, final_mask_from_agent, true_final_mask, logratio, ratio
                    else:
                        del newvalue
                    gc_cuda_cleanup()
