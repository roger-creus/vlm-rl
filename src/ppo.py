...
class Agent(nn.Module):
    ...
    def preprocess_obs(self, x):
        # x is expected to be (B, H, W, 3) or (H, W, 3) in uint8 format
        # Convert to (B, 3, H, W) float32 and normalize
        ...
        if len(x.shape) == 3:
            x = x.unsqueeze(0)
        # Permute from (B, H, W, 3) to (B, 3, H, W) if needed
...
if __name__ == "__main__":
    ...
    obs = torch.zeros((args.num_steps, args.num_envs) + envs.single_observation_space.shape, dtype=torch.uint8).to(device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        # Record video every 100 iterations
        if (iteration-1) % 100 == 0 and last_completed_episode_frames:
            try:
                pil_images = [Image.fromarray(frame) for frame in last_completed_episode_frames]
                ...
                print(f"Warning: Failed to save episode video. Error: {e}")
