...
| MiniGrid-Empty-5x5-v0 | 100 env steps (default `4 * 5 * 5`) | (160, 160, 3) uint8 via `RGBImgObsWrapper(tile_size=32)` — full-grid obs_mode | Discrete(7): LEFT / RIGHT / FORWARD / PICKUP / DROP / TOGGLE / DONE | 0.95 | `src/cleanrl_vlm/prompts/templates/minigrid/empty_5x5/actor.txt` | artifact TBD |

**Tier-1 triad complete.** Remaining envs populated as onboarded per master-spec §11 S-7.
